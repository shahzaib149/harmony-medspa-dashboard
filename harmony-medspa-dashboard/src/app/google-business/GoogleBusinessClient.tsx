...
import { Star, MessageSquare, MapPin, Phone, Globe, Image as ImageIcon, Loader2, Send, Pencil } from "lucide-react";
...
export default function GoogleBusinessClient() {
  ...
  const avgRating =
    reviews.reduce((s, r) => s + (ratingToNumber[r.starRating] ?? 0), 0) / reviews.length;
  ...
  const draftReply = async (review: Review) => {
    ...
    try {
      ...
      setDraftReplies(prev => ({ ...prev, [review.reviewId]: data.reply }));
    } catch {
      // keep existing
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-4">
        <KPICard
          ...
          color="green"
        />
        <KPICard title="Search Views" value={mockInsights.VIEWS_SEARCH.toLocaleString()} subtitle="Last 30 days" color="teal" />
        <KPICard title="Maps Views" value={mockInsights.VIEWS_MAPS.toLocaleString()} subtitle="Last 30 days" color="teal" />
        <KPICard title="Phone Calls" value={mockInsights.ACTIONS_PHONE} subtitle="From GBP listing" color="green" />
        <KPICard title="Website Clicks" value={mockInsights.ACTIONS_WEBSITE} subtitle="From GBP listing" color="teal" />
      </div>
      ...
      <div className="flex items-center border-b border-[#E5E7EB]">
        {([
          ...
          { key: "insights", label: "Insights", icon: MapPin },
      ...
      {activeTab === "reviews" && (
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex-1 space-y-1">
                {[5, 4, 3, 2, 1].map(n => {
                  const count = reviews.filter(r => ratingToNumber[r.starRating] === n).length;
                  const pct = Math.round((count / reviews.length) * 100);
                  return (
                    <div key={n} className="flex items-center gap-2 text-xs">
                      <span className="text-[#6B7280] w-3">{n}</span>
          ...
          <div className="col-span-2 space-y-4">
            {reviews.map(review => {
              const stars = ratingToNumber[review.starRating] ?? 0;
      ...
      {activeTab === "insights" && (
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-6">
            <h2 className="text-base font-semibold text-[#1A1A2E] mb-4">Search Visibility</h2>
            <div className="space-y-4">
              {[
                { label: "Direct searches (name/address)", value: mockInsights.QUERIES_DIRECT, icon: Globe, color: "#0D2B45" },
                { label: "Discovery searches (category/service)", value: mockInsights.QUERIES_INDIRECT, icon: MapPin, color: "#1A6B6B" },
                { label: "Views on Google Maps", value: mockInsights.VIEWS_MAPS, icon: MapPin, color: "#10B981" },
                { label: "Views on Google Search", value: mockInsights.VIEWS_SEARCH, icon: Globe, color: "#F59E0B" },
              ].map(item => {
                const Icon = item.icon;
                const total = mockInsights.QUERIES_DIRECT + mockInsights.QUERIES_INDIRECT + mockInsights.VIEWS_MAPS + mockInsights.VIEWS_SEARCH;
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-6">
            <h2 className="text-base font-semibold text-[#1A1A2E] mb-4">Customer Actions</h2>
            <div className="space-y-4">
              {[
                { label: "Calls from GBP", value: mockInsights.ACTIONS_PHONE, icon: Phone, color: "#1A6B6B" },
                { label: "Direction requests", value: mockInsights.ACTIONS_DRIVING_DIRECTIONS, icon: MapPin, color: "#0D2B45" },
                { label: "Website clicks", value: mockInsights.ACTIONS_WEBSITE, icon: Globe, color: "#10B981" },
                { label: "Photo views", value: mockInsights.PHOTOS_VIEWS_MERCHANT, icon: ImageIcon, color: "#F59E0B" },
              ].map(item => {
                const Icon = item.icon;
                return (
                  <div key={item.label} className="flex items-center justify-between py-2 border-b border-[#F3F4F6] last:border-0">
                    ...
                    <span className="text-lg font-bold text-[#1A1A2E] tabular-nums">{item.value.toLocaleString()}</span>
                  </div>
                );
              })}
            </div>
      ...
      {activeTab === "posts" && (
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-6">
            <h2 className="text-base font-semibold text-[#1A1A2E] mb-4">Post Tips</h2>
            <div className="space-y-3 text-sm">
              {[
                ...
                { tip: "Add a photo to every post — posts with photos get 42% more requests", color: "#F59E0B" },
                { tip: "Respond to all reviews within 24 hours — affects local ranking", color: "#1A6B6B" },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-3 py-2.5 border-b border-[#F3F4F6] last:border-0">
                  ...
                  <p className="text-[#6B7280] leading-relaxed">{item.tip}</p>
                </div>
              ))}
            </div>
          </div>
