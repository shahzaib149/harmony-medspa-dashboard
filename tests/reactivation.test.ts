...
const enrollment:Enrollment={id:"rec00000000000001",patientIds:["rec00000000000002"],campaign:DEFAULT_CAMPAIGN,status:"Active",currentStep:"Step 1 SMS",nextSendAt:"",lastSentAt:"",stopReason:"",createdAt:"2026-09-01T10:00:00Z",messagesSent:0};
const patient:Patient={id:"rec00000000000002",name:"Test Patient",phone:"",email:"",lastVisit:"2026-01-01",days:100,lastTreatment:"",status:"New",smsConsent:true,optedOut:false,doNotContact:false,futureBooking:false,replied:false,enrollments:[]};
test("every exclusion is explicit and active enrollment is campaign-specific",()=>{
  ...
  const blocked={...patient,smsConsent:false,optedOut:true,doNotContact:true,futureBooking:true,enrollments:[enrollment]};
...
test("request schema rejects malformed IDs, missing timezone, past time and oversized batches",()=>{
  ...
  for(const change of [{patientIds:["bad"]},{patientIds:[]},{patientIds:Array(201).fill(patient.id)},{campaign:""},{firstSendAt:"2099-09-15T14:00:00"},{firstSendAt:"2020-01-01T00:00:00Z"},{firstSendAt:"invalid"}]) assert.throws(()=>enrollSchema.parse({...valid,...change}));
});
test("campaign metrics exclude unrelated messages and count distinct patients",()=>{
  const msg:PatientMessage={id:"msg",patientIds:[patient.id],enrollmentIds:[enrollment.id],channel:"SMS",step:"Step 1 SMS",sentAt:"",status:"sent",body:""};
