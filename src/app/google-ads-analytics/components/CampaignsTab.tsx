...
const CHANNEL_COLORS: Record<string, string> = {
  ...
  PERFORMANCE_MAX: "#7C3AED", DEMAND_GEN: "#EA580C",
};
...
export default function CampaignsTab({ campaigns }: { campaigns: Campaign[] }) {
  ...
  const filtered = sorted.filter(c => c.campaignName.toLowerCase().includes(search.toLowerCase()));
  ...
  // Impression share
  const avgIS = campaigns.length > 0
    ? campaigns.reduce((s, c) => s + c.impressionShare, 0) / campaigns.length
    : 0;
  const avgLostBudget = campaigns.length > 0
    ? campaigns.reduce((s, c) => s + c.impressionShareLostBudget, 0) / campaigns.length
    : 0;
  const avgLostRank = campaigns.length > 0
    ? campaigns.reduce((s, c) => s + c.impressionShareLostRank, 0) / campaigns.length
    : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5">
          <h3 className="text-sm font-semibold text-[#1A1A2E] mb-4">Top Campaigns by Spend</h3>
        ...
        {/* Channel breakdown */}
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5">
          <h3 className="text-sm font-semibold text-[#1A1A2E] mb-4">Channel Type Breakdown</h3>
          <div className="space-y-4">
            {channelEntries.map(([channel, spend]) => (
              <div key={channel}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-[#374151]">{channel.replace(/_/g, " ")}</span>
                  <span className="text-sm font-medium text-[#1A1A2E]">{fmt$(spend, true)}</span>
                </div>
            ...
            {channelEntries.length === 0 && (
              <p className="text-sm text-[#9CA3AF] text-center py-8">No channel data</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Search Impression Share", value: avgIS, color: "#2563EB", desc: "% of eligible impressions received" },
          { label: "Lost to Budget", value: avgLostBudget, color: "#F59E0B", desc: "Missed due to budget constraints" },
          { label: "Lost to Ad Rank", value: avgLostRank, color: "#EF4444", desc: "Missed due to low bid or quality score" },
        ].map(item => (
          <div key={item.label} className="bg-white rounded-2xl border border-[#E5E7EB] p-5">
            <p className="text-xs font-medium text-[#6B7280] uppercase tracking-wide">{item.label}</p>
            <p className="text-3xl font-bold text-[#1A1A2E] my-2">{fmtPct(item.value)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E5E7EB] p-5">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            <h3 className="text-sm font-semibold text-[#1A1A2E]">Campaign Performance</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#F3F4F6]">
                  {["Campaign", "Status", "Type", "Spend", "Clicks", "CTR", "Conv.", "ROAS"].map(h => (
              ...
              <tbody>
                {paginated.map(c => (
                  <tr key={c.id} className="border-b border-[#F9FAFB] hover:bg-[#F9FAFB]">
                    <td className="py-2.5 pr-3 font-medium text-[#1A1A2E] max-w-[180px]">{trunc(c.campaignName, 28)}</td>
                    <td className="py-2.5 pr-3"><StatusBadge status={c.campaignStatus} /></td>
                    <td className="py-2.5 pr-3 text-[#6B7280] text-xs">{(c.channelType || "—").replace(/_/g, " ")}</td>
                    ...
                    <td className="py-2.5 pr-3">{fmtN(Math.round(c.conversions))}</td>
                    <td className="py-2.5 font-bold" style={{ color: "#1A6B6B" }}>{fmtRoas(c.roas)}</td>
                  </tr>
                ))}
                {paginated.length === 0 && (
                  <tr><td colSpan={8} className="py-8 text-center text-[#9CA3AF]">No results</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {/* Top ROAS sidebar */}
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5">
          <h3 className="text-sm font-semibold text-[#1A1A2E]">Top Performing Campaigns</h3>
          <p className="text-xs text-[#9CA3AF] mb-4 mt-0.5">By ROAS with minimum spend threshold</p>
          {topRoas.length === 0 ? (
            <p className="text-sm text-[#9CA3AF] text-center py-8">No campaigns meet the threshold</p>
          ) : (
            <div className="space-y-4">
              {topRoas.map((c, i) => (
                <div key={c.id} className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-[#1A1A2E] leading-snug">{trunc(c.campaignName, 28)}</p>
                      <p className="text-xs text-[#9CA3AF]">{fmt$(c.cost)} · {Math.round(c.conversions)} conv.</p>
                    </div>
                  </div>
                  <span className="text-lg font-bold flex-shrink-0" style={{ color: "#1A6B6B" }}>{fmtRoas(c.roas)}</span>
                </div>
              ))}
            </div>
          )}
