...
function normalizeSuggestion(value: unknown): AISuggestion | null {
  ...
  const candidate = value as Partial<AISuggestion>;
  const strings = (items: unknown) =>
    Array.isArray(items)
      ? items.filter((item): item is string => typeof item === "string")
      : [];
  const suggestion: AISuggestion = {
    headlines: strings(candidate.headlines),
    descriptions: strings(candidate.descriptions),
    displayPath: strings(candidate.displayPath),
    targetKeywords: strings(candidate.targetKeywords),
    insights: strings(candidate.insights),
    bidRecommendation:
      typeof candidate.bidRecommendation === "string"
        ? candidate.bidRecommendation
        : "",
    topPerformerSummary:
      typeof candidate.topPerformerSummary === "string"
        ? candidate.topPerformerSummary
        : "",
  };
  const hasContent =
    suggestion.headlines.length > 0 ||
    suggestion.descriptions.length > 0 ||
    suggestion.insights.length > 0 ||
    Boolean(suggestion.bidRecommendation || suggestion.topPerformerSummary);
  return hasContent ? suggestion : null;
}
...
function readHistory(): HistoryEntry[] {
  ...
  try {
    ...
    return parsed.flatMap((entry) => {
      ...
      const item = entry as { date?: unknown; suggestion?: unknown };
      const suggestion = normalizeSuggestion(item.suggestion);
      return typeof item.date === "string" && suggestion
        ? [{ date: item.date, suggestion }]
        : [];
    });
  } catch {
    return [];
...
function GoogleIcon({ size = 14 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none">
      ...
      />
      <path
        d="M6.3 14.7l7.4 5.4C15.5 16.1 19.4 13 24 13c3.1 0 5.9 1.1 8.1 2.9l6.4-6.4C34.6 4.1 29.6 2 24 2 16.3 2 9.7 7.4 6.3 14.7z"
        fill="#FF3D00"
      />
      <path
        d="M24 46c5.5 0 10.5-1.9 14.3-5.2l-6.6-5.6C29.8 36.9 27 38 24 38c-6 0-10.6-3.9-11.7-9.2l-7.4 5.7C8.3 41.7 15.6 46 24 46z"
        fill="#4CAF50"
      />
      <path
        d="M44.5 20H24v8.5h11.8c-.8 3.1-2.8 5.7-5.4 7.4l6.6 5.6C42 38.3 45 32 45 24c0-1.3-.2-2.7-.5-4z"
        fill="#1976D2"
      />
    </svg>
  );
      ...
      <div className="ad-detail-left" style={{ padding: "24px" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              >
                Harmony MedSpa · harmonymedspafl.com
              </p>
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            {[
              ...
              <span
                key={s}
                style={{ fontSize: 13, color: "#1A0DAB", cursor: "pointer" }}
              >
                {s}
              </span>
            ))}
        ...
        >
          {[
            ...
            {
              label: "Path",
              value: [r.path1, r.path2].filter(Boolean).join("/"),
              max: 30,
            },
          ].map((f) => (
            <div
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span
                  style={{
                    ...
                    flexShrink: 0,
                    color: f.value.length <= f.max ? TEAL : "#F87171",
                  }}
                >
          ...
          >
            ...
            <span
              style={{
                fontSize: 9,
                fontFamily: "monospace",
                color: r.description1.length <= 90 ? TEAL : "#F87171",
              }}
            >
              {r.description1.length}/90
            </span>
          </div>
          <p style={{ fontSize: 12, color: TEXT, margin: 0, lineHeight: 1.5 }}>
            {r.description1}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: MUTED, margin: 0, lineHeight: 1.6 }}>
            This sends the ad to Make.com, which creates a paused Responsive
            Search Ad in your Google Ads account. You can review and enable it
            there.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div
              style={{ height: 1, backgroundColor: BORDER2, margin: "2px 0" }}
            />
            <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              ...
              >
                {r.description1.length > 55
                  ? r.description1.slice(0, 55) + "…"
                  : r.description1}
              </span>
            </div>
          </div>
        ...
        {publishResult && (
          <div
            style={{
              ...
              fontSize: 11,
              lineHeight: 1.5,
              backgroundColor: publishResult.ok
                ? "rgba(45,212,191,0.08)"
                : "rgba(248,113,113,0.08)",
              borderWidth: "1px",
              borderStyle: "solid",
              borderColor: publishResult.ok
                ? "rgba(45,212,191,0.3)"
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 12, color: "#3C4043", margin: 0 }}>
              {s.descriptions?.[0]}
            </p>
          </div>
...
export default function AISuggestionsTab({
  ...
  async function runGenerate(base: HistoryEntry[] = history) {
    ...
    try {
      ...
      if (!suggestion)
        throw new Error(
          "AI returned an empty or invalid suggestion. Please regenerate.",
        );
      const newEntry: HistoryEntry = { date: todayKey, suggestion };
  ...
  const cards: UnifiedCard[] = [
    ...siteAds.map((a) => ({
      headline: [a.headline1, a.headline2, a.headline3]
        .filter(Boolean)
        .join(" | "),
      ...
      ? [
          {
            ...
            badgeColor: TEAL,
            icon: (
              <TrendingUp size={10} style={{ color: TEAL, flexShrink: 0 }} />
            ),
            raw: {
    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          <p style={{ fontWeight: 600, color: TEXT, margin: "0 0 6px" }}>
            Generating your ad suggestions…
          </p>
          <p style={{ fontSize: 13, color: MUTED, margin: 0 }}>
            Analyzing {campaigns.length} campaigns · {creatives.length} ads ·{" "}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <h3
              style={{ fontSize: 17, fontWeight: 700, color: TEXT, margin: 0 }}
            >
            ...
            {promoted && (
              ...
              >
                {fmtDate(
                  history.find((e) => e.suggestion === promoted)?.date ?? "",
                )}
              </span>
            )}
          </div>
          <p style={{ fontSize: 12, color: MUTED, margin: "4px 0 0" }}>
            Based on last {days} days · Powered by Claude
            {todayEntry && !promoted ? " · Cached for today" : ""}
          </p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {promoted && (
      ...
      {s.topPerformerSummary && (
        ...
        >
          <TrendingUp
            size={16}
            style={{ color: GOLD, marginTop: 1, flexShrink: 0 }}
          />
          <p style={{ fontSize: 13, color: TEXT, lineHeight: 1.65, margin: 0 }}>
            {s.topPerformerSummary}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <p style={{ fontSize: 11, color: MUTED, margin: "0 0 14px" }}>
            {s.headlines?.length ?? 0}/15 generated · max 30 chars
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {(s.headlines ?? []).map((h, i) => (
              ...
              >
                ...
                <span
                  style={{ fontSize: 12, color: TEXT, flex: 1, minWidth: 0 }}
                >
                  {h}
                </span>
      ...
      {s.bidRecommendation && (
        ...
        >
          <Target
            size={15}
            style={{ color: TEAL, marginTop: 1, flexShrink: 0 }}
          />
          <div>
            <p
              style={{
            ...
            <p
              style={{ fontSize: 13, color: TEXT, lineHeight: 1.6, margin: 0 }}
            >
              {s.bidRecommendation}
            </p>
          </div>
        ...
        >
          ...
          {pastEntries.map((entry) => (
            <HistoryCard
              key={entry.date}
              entry={entry}
              onPromote={() => setPromoted(entry.suggestion)}
            />
          ))}
        </div>
      )}
      ...
      >
        Powered by Claude · {history.length} day
        {history.length !== 1 ? "s" : ""} of history stored
      </p>
    </div>
  );
}
