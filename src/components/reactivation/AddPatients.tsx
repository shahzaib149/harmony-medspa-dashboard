...
export default function AddPatients({onAdded}:{onAdded:()=>void|Promise<void>}){
  ...
  const actions:[Mode,typeof Users,string,string][]=[["leads",Users,"From existing leads","Choose contacts already in your CRM."],["manual",UserPlus,"Add manually","Enter a patient's details directly."],["csv",FileSpreadsheet,"Import a CSV","Preview and add a list of patients."]];
  const count=mode==="leads"?leadIds.size:mode==="csv"?validCount:1;
  return <section className={s.panel}>
    <div className={s.toolbar}><div><h3>Add patients</h3><p className={s.muted}>Build your directory. Choose a campaign later.</p></div><span className={s.badge+" "+s.neutral}>Saved directly to Airtable</span></div>
    <div className={s.addGrid}>{actions.map(([key,Icon,title,description])=><button key={key} className={s.addCard} onClick={()=>open(key)}><span className={s.avatar}><Icon size={19}/></span><span><strong>{title}</strong><small>{description}</small></span><ArrowUpRight size={16}/></button>)}</div>
