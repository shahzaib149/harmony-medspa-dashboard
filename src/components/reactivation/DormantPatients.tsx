...
import { ArrowDownUp, ArrowUpRight, CalendarClock, CheckCircle2, ChevronLeft, ChevronRight, Download, Plus, RefreshCw, Search, Users, XCircle } from "lucide-react";
import { Toast } from "@/components/ui/Toast";
import { DEFAULT_CAMPAIGN, CLINIC_ZONE, MAX_ENROLL, MAX_PER_DAY, staggeredSendAt, activeEnrollment, currentEnrollment, exclusionReasons, nextClinicSend, clinicSendISO, type Patient, type Workspace, type EnrollmentResult } from "@/lib/reactivation/model";
...
export default function DormantPatients({initial,initialError,canManage,canDelete=false}:{initial:Workspace|null;initialError:string;canManage:boolean;canDelete?:boolean}) {
  ...
  const visible=useMemo(()=>{
    ...
    return patients.filter(p=>(!hideEnrolled||!currentEnrollment(p))&&(!statuses.length||statuses.includes(p.status))&&[p.name,p.email,p.phone,p.phone.replace(/\D/g,"")].some(v=>v.toLowerCase().includes(needle))).sort((a,b)=>{
      const value=(p:Patient):string|number=>sort.key==="enrollment"?(currentEnrollment(p)?.currentStep||"Not enrolled"):sort.key==="emailConsent"?Number(p.emailConsent):sort.key==="days"?(p.days??-1):p[sort.key];
  ...
  function enrollOne(id:string){setSelected(new Set([id]));setPatientId(null);void openReview([id]);}
  async function openReview(ids: string[] = [...selected]){
    ...
    try{
      ...
      setData({...fresh,patients:[...fresh.patients,...patients.filter(p=>ids.includes(p.id)&&!fresh.patients.some(n=>n.id===p.id))]});
  ...
  return <div className={s.workspace}>
    <div className={s.kpis}>{([["Patients",patients.length,"In the patient directory"],["Ready to enroll",ready,"Contact checks passed"],["In reactivation",patients.filter(p=>activeEnrollment(p)).length,"Active enrollments"],["Away 180+ days",patients.filter(p=>(p.days??0)>=180).length,"Known last visit"]] as const).map(([label,value,hint])=><article key={label} className={s.kpi}><span>{label}</span><strong>{data?value.toLocaleString("en-US"):<Skeleton className="my-1 h-7 w-16 rounded-md"/>}</strong><small>{hint}</small></article>)}</div>
    {toast&&<Toast {...toast} onClose={()=>setToast(null)}/>}
    {result&&<div className={s.notice} role="status"><div className="flex flex-wrap justify-between gap-3"><strong>{result.created} patients enrolled · {result.skipped.length} skipped</strong><div className="flex flex-wrap gap-3"><button className={s.linkButton} onClick={()=>downloadCsv("reactivation-enrollment-"+new Date().toISOString().slice(0,10)+".csv",resultRows)}><Download size={14}/>Download results</button><button onClick={()=>setResult(null)} aria-label="Dismiss enrollment results">Dismiss</button></div></div>{result.skipped.length>0&&<ul className={s.exclusions}>{result.skipped.map(p=><li key={p.id}><strong>{p.name}</strong> — {p.reason}</li>)}</ul>}</div>}
    <section className={s.panel}>
      <div className={s.toolbar}><div><h3>Patient directory <span className={s.badge+" "+s.gold}>{visible.length}</span></h3><p className={s.muted}>{data?.source||<Skeleton className="mt-1 h-3 w-40 rounded-full"/>}</p></div><div className={s.toolbarActions}>{canManage&&<AddPatients onAdded={load}/>}<button className={s.button} onClick={()=>load()} disabled={loading}><RefreshCw size={14} className={loading?"animate-spin":""}/>Refresh</button>{canManage&&<button className={s.button+" "+s.primary} onClick={()=>void openReview()} disabled={!selected.size||loading}><Users size={15}/>Enroll selected in Reactivation{selected.size>0?" ("+selected.size+")":""}</button>}</div></div>
      ...
      <div className={s.mobileControls}>
        <label className={s.toggle}><input type="checkbox" aria-label="Select patients on this page" disabled={!canManage} checked={pageRows.length>0&&pageSelected===pageRows.length} onChange={e=>setSelected(old=>{const next=new Set(old);pageRows.forEach(p=>e.target.checked?next.add(p.id):next.delete(p.id));return next;})}/>Select page</label>
        <label className={s.label}>Sort by<select className={s.input} value={sort.key} onChange={e=>{setSort({key:e.target.value as SortKey,desc:sort.desc});setPage(1);}}>{columns.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
        <button className={s.button} onClick={()=>{setSort({...sort,desc:!sort.desc});setPage(1);}}>{sort.desc?"Descending":"Ascending"}</button>
      </div>
      {error?<div className={s.empty} role="alert"><XCircle size={26}/><h3>Patient records couldn’t be loaded</h3><p className={s.muted}>{error}</p><button className={s.button+" mt-4"} onClick={()=>load()}>Try again</button></div>:loading?<LoadingRegion label="Loading patient records" className={s.tableWrap}><SkeletonRows rows={8}/></LoadingRegion>:!visible.length?<div className={s.empty}><Search size={28}/><h3>No patients match these filters</h3><p className={s.muted}>Add patients above, or adjust the search, status, and enrollment filters.</p></div>:<div className={s.tableWrap}><table role="table" aria-label="Patient directory" className={s.table+" "+s.patientTable}><thead><tr><th><input ref={allCheck} type="checkbox" aria-label="Select all patients on this page" disabled={!canManage} checked={pageRows.length>0&&pageSelected===pageRows.length} onChange={e=>setSelected(old=>{const next=new Set(old);pageRows.forEach(p=>e.target.checked?next.add(p.id):next.delete(p.id));return next;})}/></th>{columns.map(([key,label])=><th key={key} aria-sort={sort.key===key?(sort.desc?"descending":"ascending"):"none"}><button onClick={()=>{setSort({key,desc:sort.key===key?!sort.desc:false});setPage(1);}}>{label}<ArrowDownUp size={11}/></button></th>)}</tr></thead><tbody role="rowgroup">{pageRows.map(p=>{const active=currentEnrollment(p);return <tr role="row" key={p.id} onClick={()=>setPatientId(p.id)} className={selected.has(p.id)?s.selected:""}><td role="cell" data-label="Select patient" onClick={e=>e.stopPropagation()}><input type="checkbox" disabled={!canManage} aria-label={"Select "+p.name} checked={selected.has(p.id)} onChange={()=>toggle(p.id)}/></td><td role="cell" data-label="Patient"><button className={s.person} onClick={e=>{e.stopPropagation();setPatientId(p.id);}}><span className={s.avatar}>{p.name.split(" ").slice(0,2).map(n=>n[0]).join("")}</span><strong>{p.name}</strong><ArrowUpRight size={12} className="opacity-40"/></button></td><td role="cell" data-label="Phone">{p.phone||"—"}</td><td role="cell" data-label="Email">{p.email||"—"}</td><td role="cell" data-label="Last visit">{date(p.lastVisit)}</td><td role="cell" data-label="Days away">{p.days===null?<span className={s.badge+" "+s.neutral} title="No last visit date recorded">Unknown</span>:<span className={s.days}><strong>{p.days}</strong><span className={s.bar}><i style={{width:Math.min(p.days/365*100,100)+"%"}}/></span></span>}</td><td role="cell" data-label="Last treatment">{p.lastTreatment||"—"}</td><td role="cell" data-label="Status"><span className={s.badge+" "+s.neutral}>{p.status}</span></td><td role="cell" data-label="Email consent"><span className={s.badge+" "+(!p.emailConsent?s.neutral:"")}>{p.emailConsent?<CheckCircle2 size={12}/>:null}{p.emailConsent?"Confirmed":"Not recorded"}</span></td><td role="cell" data-label="Enrollment">{active?<div><span className={s.badge}>{active.status}</span><p className="mt-1 text-xs">{active.currentStep}</p></div>:<span className={s.muted}>Not enrolled</span>}</td></tr>;})}</tbody></table></div>}
