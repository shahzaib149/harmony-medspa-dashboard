...
export default function DormantPatients({initial,initialError,canManage}:{initial:Workspace|null;initialError:string;canManage:boolean}) {
  ...
  const load=useCallback(async(all=scope)=>{
    const version=++loadVersion.current;setLoading(true);setError("");
    try {const next=await json<Workspace>("/api/reactivation/patients"+(all?"?scope=all":""));if(version!==loadVersion.current)return;setData(next);setScope(all);setSelected(new Set());setCampaign(c=>next.campaigns.includes(c)?c:next.campaigns[0]||"");}
  ...
  const visible=useMemo(()=>{
    ...
    return patients.filter(p=>(p.days??-1)>=threshold&&(!consent||p.smsConsent)&&(!hideEnrolled||!activeEnrollment(p))&&(!statuses.length||statuses.includes(p.status))&&[p.name,p.email,p.phone,p.phone.replace(/\D/g,"")].some(v=>v.toLowerCase().includes(needle))).sort((a,b)=>{
  ...
  },[patients,query,threshold,consent,hideEnrolled,statuses,sort]);
  ...
  async function openReview(){
    ...
    try{
      ...
      setScope(true);setCampaign(c=>fresh.campaigns.includes(c)?c:fresh.campaigns[0]||"");
  ...
  async function enroll(){
    ...
    try{
      ...
      setResult(next);setToast({message:next.created+" enrolled · "+next.skipped.length+" skipped",variant:next.skipped.length?"warning":"success"});setModal(false);setSelected(new Set());await load(true);router.refresh();
  ...
  return <div className={s.workspace}>
    ...
    <section className={s.panel}>
      ...
      <div className={s.filters}>
        ...
        <label className={s.label}>Time since last visit<select className={s.input} value={threshold} onChange={e=>{setThreshold(Number(e.target.value));if(Number(e.target.value)===60&&!scope)void load(true);}}>{[60,90,180].map(n=><option key={n} value={n}>{n}+ days</option>)}</select></label>
        <label className={s.toggle}><input type="checkbox" checked={consent} onChange={e=>{setConsent(e.target.checked);if(!e.target.checked&&!scope)void load(true);}}/>SMS consent only</label>
        <label className={s.toggle}><input type="checkbox" checked={hideEnrolled} onChange={e=>{setHideEnrolled(e.target.checked);if(!e.target.checked&&!scope)void load(true);}}/>Hide already enrolled</label>
      </div>
