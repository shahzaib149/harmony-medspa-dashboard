...
export default function PatientDrawer({ patientId, onClose, canManage, canDelete, onChanged, enrollSlot }: {
  ...
  return <PatientDialog open={Boolean(patientId)} onClose={() => { if (!busy && !actionBusy) onClose(); }} title={patient?.name || "Patient details"} eyebrow="Patient profile" drawer busy={busy || Boolean(actionBusy)}>
    ...
    {patient && <>
      ...
      <section><h3 className={s.sectionTitle}>Enrollment history · {patient.enrollments.length}</h3>{!patient.enrollments.length ? <p className={s.muted}>This patient hasn’t been enrolled in reactivation.</p> : <div className={s.timeline}>{patient.enrollments.map(e => <article key={e.id} className={s.history}><div className="flex justify-between gap-2"><h4>{e.campaign}</h4><span className={s.badge + " " + (e.status === "Active" ? "" : s.neutral)}>{e.status}</span></div><p>{e.currentStep}</p><p className={s.muted}>Enrolled: {date(e.createdAt, true)}<br />Next send: {date(e.nextSendAt, true)}<br />Last sent: {date(e.lastSentAt, true)}<br />Stop reason: {e.stopReason || "—"}</p>{["Active", "Paused"].includes(e.status) && canManage && (stopId === e.id ? <div className={s.notice}><p>Remove this patient from the campaign? Scheduled sends will be cleared; enrollment and message history will remain.</p><div className="mt-3 flex flex-wrap gap-2"><button className={s.button} disabled={busy} onClick={() => setStopId(null)}>Cancel</button><button className={s.button} disabled={busy} onClick={stop}>{busy ? "Stopping…" : "Remove from campaign"}</button></div></div> : <button className={s.button + " mt-3"} disabled={busy || Boolean(actionBusy)} onClick={() => setStopId(e.id)}>Remove from campaign</button>)}</article>)}</div>}</section>
