...
export default function CallLeadsPanel({
  ...
  const loadAddedCalls = useCallback(async () => {
    try {
      ...
      const text = (body.items ?? [])
        ...
        .join("\n");
      setAddedIds(new Set(
        CALLS.filter((call) => text.includes(marker(call.id))).map((call) => call.id),
      ));
    } finally {
      setChecking(false);
    }
  ...
  const summary = useMemo(() => ({
    ...
    averageDuration: Math.round(CALLS.reduce((sum, call) => sum + call.duration, 0) / CALLS.length),
  }), []);
  ...
  async function submit(event: React.FormEvent) {
    ...
    const callContext = [
      marker(selected.id),
      "Inbound CS2 call: " + selected.date + ", 2026 at " + selected.time + " ET",
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-[var(--brand-primary-soft)] px-2 py-1 text-[10px] font-extrabold uppercase tracking-[0.1em] text-[var(--brand-primary-strong)]">CS2</span>
            </div>
            <p className="mt-0.5 text-xs text-[var(--text-muted)]">Inbound call opportunities · Aug 31–Sep 9, 2026</p>
          </div>
        </div>
            ...
            {CALLS.map((call) => {
              ...
              return (
                <tr key={call.id} className="transition-colors hover:bg-[var(--surface-hover)]">
                  ...
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center gap-1.5 text-xs tabular-nums text-[var(--text-secondary)]"><Clock3 size={13} />{call.duration}s</span>
                  </td>
                  <td className="px-4 py-3">
          ...
          <div className="relative flex h-dvh max-h-dvh w-full max-w-[620px] flex-col overflow-hidden border border-[var(--border-strong)] bg-[var(--surface-raised)] shadow-[var(--shadow-modal)] sm:h-auto sm:max-h-[92dvh] sm:rounded-3xl">
            ...
            <form onSubmit={submit} className="flex-1 space-y-4 overflow-y-auto p-5 sm:p-6">
              <div className="grid grid-cols-3 gap-2 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-2)] p-3 text-center">
                ...
                <CallFact label="Duration" value={selected.duration + "s"} />
