...
export default function NurtureDetail({ enrollment, onClose }: { enrollment: NurtureEnrollment | null; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-[520px] flex-col border-l bg-[var(--background)]" style={{ borderColor: "var(--border-subtle)", boxShadow: "-28px 0 80px rgba(0,0,0,.45)" }}>
        <div className="flex items-start justify-between border-b p-5" style={{ borderColor: "var(--border-subtle)" }}>
          <div><p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#C9A84C]">Nurture profile</p><h2 className="mt-1 text-xl font-extrabold text-[var(--text-primary)]">{enrollment.leadName}</h2><p className="mt-1 text-xs text-[var(--text-muted)]">{enrollment.leadSource || "Unknown source"} · {enrollment.leadTreatmentInterest || "No treatment captured"}</p></div>
          <button onClick={onClose} className="rounded-xl border p-2 text-[var(--text-muted)]" style={{ borderColor: "var(--border-subtle)" }}><X size={17} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-5">
          <div className="grid grid-cols-2 gap-3">
            {[{ label: "Phone", value: enrollment.leadPhone, icon: Phone }, { label: "Email", value: enrollment.leadEmail, icon: Mail }, { label: "Lead status", value: enrollment.leadStatus, icon: Sparkles }, { label: "Replied", value: enrollment.leadReplied ? "Yes" : "No", icon: MessageSquare }].map(({ label, value, icon: Icon }) => (
              <div key={label} className="rounded-2xl border p-3" style={{ backgroundColor: "var(--surface-1)", borderColor: "var(--border-subtle)" }}><p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[.08em] text-[var(--text-muted)]"><Icon size={11} />{label}</p><p className="mt-1 break-words text-sm font-semibold text-[var(--text-primary)]">{value || "Not captured"}</p></div>
            ))}
          </div>
