...
export type Patient = { id: string; name: string; phone: string; email: string; lastVisit: string; days: number | null; lastTreatment: string; status: string; smsConsent: boolean; optedOut: boolean; doNotContact: boolean; futureBooking: boolean; replied: boolean; enrollments: Enrollment[] };
...
export const enrollSchema = {
  parse(value: unknown): { patientIds: string[]; campaign: string; firstSendAt: string } {
    ...
    if (!Array.isArray(patientIds) || patientIds.length < 1 || patientIds.length > 200 || !patientIds.every(id => typeof id === "string" && /^rec[a-zA-Z0-9]{14}$/.test(id))) throw new Error("Select between 1 and 200 valid patients.");
